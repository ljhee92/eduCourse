...
public class StdntTestDAO {
	...
	public List<StdntTestVO> slctAllStdntTestList(int stdnt_number) throws SQLException {
		...
		try {
			...
			slctAllStdntTestList.append(
					"select d.dept_name, c.course_name, r.course_code, p.prof_name, l.test_flag, s.score, c.credit_hours ")
					.append("from register r ").append("join course c on c.course_code = r.course_code ")
			...
			while (rs.next()) {
				stVO = new StdntTestVO(rs.getString("dept_name"), rs.getString("course_name"),
						rs.getString("course_code"), rs.getString("prof_name"), rs.getString("test_flag"),
						rs.getInt("score"), rs.getInt("credit_hours"));
				listSTVO.add(stVO);
			} // end while
		} finally {
	...
	public ScoreVO slctScore(int stdnt_number, String course_code) throws SQLException {
		...
		try {
			...
			slctOneScore.append("select r.register_number, s.score ").append("from register r ")
