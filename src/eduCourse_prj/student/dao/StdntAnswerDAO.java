...
public class StdntAnswerDAO {
	...
	public StdntAnswerVO slctExamAnswer(String course_code) throws SQLException {
		...
		try {
			...
			while (rs.next()) {
				saVO = new StdntAnswerVO(rs.getInt("question_number"), rs.getString("std_answer"),
	...
	public List<SubStdntAnswerVO> slctExamCrsList(int std_number) throws SQLException {
		...
		try {
			...
			slctQuery.append("SELECT r.register_number,c.course_name,c.course_code ").append("FROM register r ")
			...
			while (rs.next()) {
				ssaVO = new SubStdntAnswerVO(rs.getInt("register_number"), rs.getString("course_code"),
	...
	public List<StdntAnswerVO> slctExamAnswer(String course_code, int register_number) throws SQLException {
		...
		try {
			...
			StringBuilder slctQuery = new StringBuilder();
			slctQuery.append("SELECT s.question_number, s.std_answer, t.answer ").append("FROM std_answer s ")
